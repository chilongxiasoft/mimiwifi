...
class AuthManagerController extends AdminController{
    ...
    public function updateRules(){
        ...
        $AuthRule = M('AuthRule');
        $map      = array('module'=>'home','type'=>array('in','1,2'));//status全部取出,以进行更新
    ...
    public function user($group_id){
        ...
        $_REQUEST = array();
        $list = $this->lists($model,array('a.group_id'=>$group_id,'m.status'=>array('egt',0)),'m.uid asc',null,'m.uid,m.username,m.nickname,m.usertype,m.login,last_login_time,m.last_login_ip,m.status');
        int_to_string($list);
        $this->assign( '_list',     $list );
        $this->assign('auth_group', $auth_group);
        $this->assign('this_group', $auth_group[(int)$_GET['group_id']]);
