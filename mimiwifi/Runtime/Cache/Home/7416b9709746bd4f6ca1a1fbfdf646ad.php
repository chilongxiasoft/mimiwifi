  ...
  <div class="page-header">
        ...
        <div class="widget-content no-padding">
          <div class="row">
            <div class="dataTables_header clearfix">
              <div class="col-md-6"> <a class="btn" href="<?php echo U('createGroup');?>">新 增</a> 
			  <a class="btn ajax-post" url="<?php echo U('changestatus',array('method'=>'resumeGroup'));?>" target-form="ids">启 用</a> 
			  <a class="btn ajax-post" url="<?php echo U('changestatus',array('method'=>'forbidGroup'));?>" target-form="ids">禁 用</a>
			  <a class="btn ajax-post" url="<?php echo U('changestatus',array('method'=>'deleteGroup'));?>" target-form="ids">删 除</a> </div>
              <div class="col-md-6">
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><i class="icon-key "></i> <a href="<?php echo U('AuthManager/access?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" >系统权限</a>
			<i class="icon-user "></i> <a href="<?php echo U('AuthManager/user?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" >成员授权</a>
			</td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<i class="icon-edit"></i> <a href="<?php echo U('AuthManager/editgroup?id='.$vo['id']);?>">编辑</a>
				<?php if(($vo["status"]) == "1"): ?><i class="icon-lock"></i> <a href="<?php echo U('AuthManager/changeStatus?method=forbidGroup&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<i class="icon-unlock"></i> <a href="<?php echo U('AuthManager/changeStatus?method=resumeGroup&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<i class="icon-trash"></i> <a href="<?php echo U('AuthManager/changeStatus?method=deleteGroup&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
