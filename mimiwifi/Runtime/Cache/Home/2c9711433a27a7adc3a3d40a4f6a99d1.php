         ...
         <div class="row">
            <div class="dataTables_header clearfix">
				<div class="col-md-6">
					...
					<a class="btn ajax-post" url="<?php echo U('del?id='.$config['id']);?>" target-form="ids">删 除</a>
					<button class="btn list_sort" url="<?php echo U('sort?group='.I('group'),'','');?>">排序</button>
				</div>
              <div class="col-md-6 dataTables_filter">
                <?php if(isset($_GET['group'])): ?><a class="btn btn-info" href="<?php echo U('index');?>">全部</a><?php else: ?><button class="btn  btn-success" >全部</button><?php endif; ?>
				<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a class="btn btn-info" href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><button class="btn btn-success"><?php echo ($vo); ?></button><?php endif; ?>&nbsp;<?php endforeach; endif; ?>
              </div>
            </div>
         </div>
		<table class="table table-striped table-bordered table-hover table-checkable">
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><tr>
						<td class="checkbox-column"><input type="checkbox" class="uniform ids" name="id[]" value="<?php echo ($config["id"]); ?>" /></td>
						...
						<td>
							<i class="icon-edit"></i> <a title="编辑" href="<?php echo U('edit?id='.$config['id']);?>">编辑</a>
							<i class="icon-trash"></i> <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$config['id']);?>">删除</a>
						</td>
