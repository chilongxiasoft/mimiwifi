      ...
      <div class="widget box">
       <div class="widget-content no-padding">
          <div class="row">
            ...
            <?php if(is_array(C("CONFIG_GROUP_LIST"))): $i = 0; $__LIST__ = C("CONFIG_GROUP_LIST");if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><a  class="btn <?php if(($id) == $key): ?>btn-success
              <?php else: ?>
              btn-info<?php endif; ?>
              " href="<?php echo U('?id='.$key);?>"><?php echo ($group); ?>配置</a>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
			</div>
		</div>

        <form action="<?php echo U('save');?>" method="post" class="form-horizontal row-border">
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><span></span>
            <div class="form-group">
              <label class="col-md-3 control-label"><?php echo ($config["title"]); ?></label>
              <div class="col-md-9">
                <?php switch($config["type"]): case "0": ?><input type="text" class="form-control input-width-small" name="config[<?php echo ($config["name"]); ?>]" value="<?php echo ($config["value"]); ?>"><?php break;?>
                  <?php case "1": ?><input type="text" class="form-control input-width-large" name="config[<?php echo ($config["name"]); ?>]" value="<?php echo ($config["value"]); ?>"><?php break;?>
                  <?php case "2": ?><div class="col-md-5">
                      <textarea class="form-control" rows="5" name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea>
                    </div><?php break;?>
                  <?php case "3": ?><div class="col-md-5">
                      <textarea class="form-control" rows="5" name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea>
                    </div><?php break;?>
                  <?php case "4": ?><div class="col-md-5">
                ...
                <span class="help-block">（<?php echo ($config["remark"]); ?>）</span> </div>
            </div><?php endforeach; endif; else: echo "" ;endif; ?>
          <div class="form-group">
            <div class="col-md-offset-4">
              <?php if(empty($list)): ?><button type="submit" disabled class="btn submit-btn disabled" target-form="form-horizontal">确 定</button>
                <?php else: ?>
                <button type="submit" class="btn submit-btn ajax-post" target-form="form-horizontal">确 定</button><?php endif; ?>
