        ...
        <div class="widget-content no-padding">
         <div class="row">
            <div class="dataTables_header clearfix">
				<div class="col-md-6">
					<a class="btn btn-primary" href="<?php echo U('User/add');?>">添加用户</a>
					<a class="btn ajax-post" url="<?php echo U('User/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</a>
					<a class="btn ajax-post" url="<?php echo U('User/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</a>
					<a class="btn ajax-post" url="<?php echo U('User/delete?id=');?>" target-form="ids">删 除</a>
				</div>
              <div class="col-md-6 dataTables_filter">
                  ...
                  <div class="input-group search-form"> <span class="input-group-addon"><a id="search" href="javascript:;" id="search" url="<?php echo U('index');?>"><i class="icon-search"></i></a></span>
                    <input type="text" class="form-control search-input" placeholder="请输入用户昵称或者ID" name="nickname" value="<?php echo I('nickname');?>">
                  </div>
                  </label>
              </div>
            </div>
          ...
          <table class="table table-striped table-bordered table-hover table-checkable align-center">
            ...
            <tbody>
                <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                  <td class="checkbox-column"><input type="checkbox" class="uniform ids" name="id[]" value="<?php echo ($vo["uid"]); ?>" /></td>
                  <td><?php echo ($vo["uid"]); ?></td>
                  <td><?php echo ($vo["username"]); ?></td>
                  <td><?php echo ($vo["nickname"]); ?></td>
                  <td><?php echo ($vo["usertype_text"]); ?></td>
				  <td class="text-left">
				  设备: 5 台<span><i class="icon-link"></i> <a href="<?php echo U('User/edit?id='.$vo['uid']);?>" class="confirm-dialog">绑定</a></span></br>
				  应用点: 3 个<span><i class="icon-sitemap"></i> <a href="<?php echo U('User/edit?id='.$vo['uid']);?>" class="confirm-dialog">添加</a></span></br>
				  云认证: 1 个<span><i class="icon-cloud"></i> <a href="<?php echo U('User/edit?id='.$vo['uid']);?>" class="confirm-dialog">设置</a></span></br>
				  余额: 100 元 <span><i class="icon-plus"></i> <a href="<?php echo U('User/edit?id='.$vo['uid']);?>" class="confirm-dialog">充值</a></span></td>
                  <td class="text-left">今日: </br>昨日: </br>历史: </td>
                  <td class="text-left"><?php echo (time_format($vo["last_login_time"])); ?></br>IP: <a target=_blank href="http://ip.chinaz.com/?IP=<?php echo long2ip($vo['last_login_ip']);?>"><?php echo long2ip($vo['last_login_ip']);?></a></br>累计登录:<?php echo ($vo["login"]); ?></td>
                  <td><?php echo ($vo["status_text"]); ?></td>
                  <td>
                    <i class="icon-edit"></i> <a href="<?php echo U('User/edit?id='.$vo['uid']);?>" class="confirm-dialog">修改</a>
					<i class="icon-trash"></i> <a href="<?php echo U('User/delete?id='.$vo['uid']);?>" class="confirm-dialog">删除</a>
					<?php if(($vo["status"]) == "1"): ?><i class="icon-lock"></i> <a href="<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['uid']);?>">锁定</a>
                    <?php else: ?>
                    <i class="icon-unlock"></i> <a href="<?php echo U('User/changeStatus?method=resumeUser&id='.$vo['uid']);?>">启用</a><?php endif; ?>
					<br>
					<i class="icon-star-empty"></i> <a href="<?php echo U('AuthManager/addToGroup?group_id=1&uid='.$vo['uid']);?>">设用</a>
					<i class="icon-star"></i> <a href="<?php echo U('AuthManager/addToGroup?group_id=2&uid='.$vo['uid']);?>">设代</a>
					<i class="icon-asterisk"></i> <a href="<?php echo U('AuthManager/addToGroup?group_id=3&uid='.$vo['uid']);?>">设管</a>
				  </td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
              <?php else: ?>
            <td colspan="10">暂时还没有内容! </td><?php endif; ?>
                </tbody>
          </table>
          <div class="row">
